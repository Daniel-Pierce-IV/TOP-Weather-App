export default {
  clear(classes) {
    return `<svg class="${classes}" viewBox="0 0 24 24">
  ...
  fromCode(code, svgClasses) {
    // Codes taken from https://openweathermap.org/weather-conditions
    if (code >= 200 && code < 300) return this.storm(svgClasses);
    if (code >= 300 && code < 400) return this.rain(svgClasses);

    // There are no 4xx codes
    // 5xx is drizzle, but calling it rain for simplicity
    if (code >= 500 && code < 600) return this.rain(svgClasses);
    if (code >= 600 && code < 700) return this.snow(svgClasses);

    // 7xx is actually atmosphere, but we're not supporting those
    if (code >= 700 && code < 800) return this.clear(svgClasses);
    if (code === 800) return this.clear(svgClasses);
    if (code > 800 && code <= 804) return this.clouds(svgClasses);
